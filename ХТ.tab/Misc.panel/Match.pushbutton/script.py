...
__title__ = 'Сопоставление\nпереопределения\nграфики'
__author__ = 'pyrevit (mod.SG)'
...
def pick_and_match_styles(src_style, targets=None):
    with forms.WarningBar(title='Pick objects to match overrides:'):
        while True:
            if targets:
                dest_element = targets
                dest_element_ids = [el.Id for el in targets]
                for el in targets:
...
targets = [doc.GetElement(elid) for elid in uidoc.Selection.GetElementIds()]
...
if source_element:
    if isinstance(source_element, DB.Dimension):
        ...
        pick_and_match_styles(source_style, targets)
